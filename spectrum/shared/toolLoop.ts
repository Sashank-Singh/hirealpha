  ...
  for (let step = 0; step <= maxSteps; step++) {
    ...
    if (!lookup && !draft && !directive) {
      ...
      if (publicMatches.size && !savedDraft && ![...publicMatches.keys()].some(url => raw.includes(url))) {
        ...
        sourcesNudged = true
        messages.push({ role: 'assistant', content: raw })
        messages.push({ role: 'system', content: 'Your answer has no retrieved source links. Correct it before delivery: recommend at most two options and include their actual source URLs. A directory listing only establishes the names listed; it does not establish cuisine, atmosphere, price, address, or quality. If those details are missing, search for the shortlisted restaurants’ official websites and addresses in the requested area. Do not fill missing details from memory. If you cannot verify them, return only the supported names and links.' })
        continue
      }
