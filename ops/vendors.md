...
| Ollama (self-hosted) | Embedding model for conversational-memory recall (`services/trust/memoryIndex.ts`, `OLLAMA_BASE_URL`, model `qwen3-embedding:0.6b`); self-hosted on the same VPS, so it is infrastructure rather than an external subprocessor — **memory text is not sent to any third party to be embedded** | Memory fact text (`key: value`) is embedded locally; nothing leaves the VPS. This is the reason the embedder is Ollama rather than a hosted embeddings API: it avoids creating an unsigned-DPA subprocessor that receives user memory, and it makes recall work with no egress | N/A — self-hosted. Note the model pull itself needs outbound access to the Ollama registry once; no customer data is involved in that fetch | Same VPS (`vmi2997871`) |
| Mem0 (`mem0ai`) | Retrieval library for the memory index, run in-process inside `hire-api`. It is a **library, not a service**: there is no Mem0 endpoint and no Mem0 account | Nothing is sent to mem0ai the company at runtime. Telemetry is force-disabled (`process.env.MEM0_TELEMETRY = 'false'` set in `services/trust/memoryIndex.ts` before the module loads) — their default ships an `init` event to PostHog on every Memory construction, awaited during initialisation, which is both a third-party data flow and a hang risk on a restricted-egress host | N/A — in-process library, no subprocessor | N/A |
